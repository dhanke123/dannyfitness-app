...
/* PayNow QR. If the admin has uploaded the real bank-generated QR (Settings →
   PayNow details) it renders that image; otherwise the placeholder pattern.
   `uen` prop (admin-set) beats the env var. */
export const QR = ({ uen, image }) => (
  <div className="mx-auto my-2 p-3 rounded-xl" style={{background:"#fff",border:`1.5px solid ${T.line}`,width:150}}>
    {image ? (
      <img src={image} alt="PayNow QR" style={{width:120,height:120,objectFit:"contain",display:"block"}}/>
    ) : (
    ...
    <div className="text-center text-[10px] mt-1.5 font-bold" style={{...body,color:"#7B1FA2"}}>PAYNOW · UEN {uen || _UEN}</div>
  </div>);
